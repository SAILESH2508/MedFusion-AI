...
import { getRiskColor, getRiskBg, getAge, getHasShownGuestPopup, setHasShownGuestPopup } from '../../services/utils';
...
function GuestDashboard({ openAuth }) {
  ...
  // Guest Profile & Consolidated Records State
  ...
  const fetchGuestHistory = useCallback(() => {
    ...
    api.get(`/predictions/?email=${guestEmail}`)
      .then(res => setPredictionsHistory(res.data))
      .catch(err => console.error("Error fetching predictions:", err));

    api.get(`/prescriptions/?email=${guestEmail}`)
      .then(res => setPrescriptions(res.data))
      .catch(err => console.error("Error fetching prescriptions:", err));

    api.get(`/pathology/?email=${guestEmail}`)
  ...
  useEffect(() => {
    ...
    const displayedPredictors = allPredictors;
        ...
        <div className="col-12 col-xl-10 mx-auto reveal">
          <AnimatePresence mode="wait">
            ...
            {!result && !isAnalyzing && (
              ...
              >
                {/* Guest Profile Information Card */}
                <div className="glass-card p-4">
                  ...
                  <p className="text-secondary font-monospace mb-3" style={{ fontSize: '0.85rem', lineHeight: '1.4' }}>
                    Guest Portal: Enter your name and email to save your diagnostic predictions permanently in the local database.
                  </p>
                  <div className="row g-3">
                    <div className="col-6">
                      <div className="predictor-card" style={{ background: 'rgba(255, 255, 255, 0.02)', padding: '15px', borderRadius: '8px', border: '1px solid rgba(255, 255, 255, 0.05)' }}>
                        <label className="small text-secondary fw-semibold mb-2 font-monospace d-block">Full Name</label>
                        <input 
                          type="text" 
                          placeholder="e.g. John Doe"
                          value={guestName}
                          onChange={(e) => { setGuestName(e.target.value); setHasChanges(true); }}
                          style={{
                            width: '100%',
                    ...
                    <div className="col-6">
                      <div className="predictor-card" style={{ background: 'rgba(255, 255, 255, 0.02)', padding: '15px', borderRadius: '8px', border: '1px solid rgba(255, 255, 255, 0.05)' }}>
                        <label className="small text-secondary fw-semibold mb-2 font-monospace d-block">Email Address (Key)</label>
                        <input 
                          type="email" 
                          placeholder="e.g. john.doe@example.com"
                          value={guestEmail}
                          onChange={(e) => { setGuestEmail(e.target.value); setHasChanges(true); }}
                          style={{
                            width: '100%',
                            background: 'rgba(255, 255, 255, 0.05)',
                            border: '1px solid rgba(255, 255, 255, 0.1)',
